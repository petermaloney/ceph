		       ...
		       int uid, int gid)
{
  ...
  unsigned mask = statx_to_mask(flags, want);
  r = _lookup(parent, dname, mask, &in, uid, gid);
  if (r < 0) {
    stx->stx_ino = 0;
    stx->stx_mask = 0;
  } else {
		     ...
		     unsigned int want, unsigned int flags)
{
  ...
  ldout(cct, 3) << "ll_walkx" << name << dendl;
  tout(cct) << "ll_walkx" << std::endl;
  tout(cct) << name << std::endl;
		      ...
		      int uid, int gid)
{
  ...
  bool created = false;
  int r = _lookup(parent, name, caps, in, uid, gid);
		      ...
		      int uid, int gid)
{
  ...
  int r = _ll_create(parent, name, mode, flags, &in, CEPH_STAT_CAP_INODE_ALL, fhp, uid, gid);
  if (r >= 0) {
    assert(in);
